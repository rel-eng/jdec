module JDec.Class.Raw.ExceptionHandlerInfo (
...
data ExceptionHandlerInfo = ExceptionHandlerInfo {
  ...
  catchType :: ConstantPoolIndex -- ^ If the value is nonzero, it must be a valid index into the constant pool table. The constant pool entry at that index must be a ClassConstantPoolEntry representing a class of exceptions that this exception handler is designated to catch. The exception handler will be called only if the thrown exception is an instance of the given class or one of its subclasses. If the value is zero, this exception handler is called for all exceptions.
} deriving Show
